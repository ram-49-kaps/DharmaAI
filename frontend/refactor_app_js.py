...
new_handle_send = '''  const handleSend = async (text, attachments = []) => {
    ...
    const onChunk = (parsed) => {
      setSession((prev) => {
        ...
        const lastMsg = { ...msgs[msgs.length - 1] }; // the assistant message
        ...
        if (parsed.type === "metadata") {
          ...
          // We could track status if we wanted to display it
        }
...
start_send = content.find('  const handleSend = async (text, attachments = []) => {')
end_send = content.find('  // ── Edit message', start_send)
start_edit = content.find('  const handleEditMessage = async (index, newContent) => {', end_send)
end_edit = content.find('  // ── Share & New Chat', start_edit)

if start_send != -1 and end_send != -1 and start_edit != -1 and end_edit != -1:
    content = content[:start_send] + new_handle_send + '\n\n' + content[end_send:start_edit] + new_handle_edit + '\n\n' + content[end_edit:]
    with open("src/App.js", "w") as f:
        f.write(content)
    print("Replaced handleSend and handleEditMessage")
else:
